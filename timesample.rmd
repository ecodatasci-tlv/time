...
```{r}
timeSample <- data.frame(
  data_1 = c("02-10-2019", "2/10/19"),
  data_2 = c("2019 10 02", "19/10/2"),
  datatime_1 = c("02-10-2019 19:00:00", "02-10-2019 7:00:00PM")
)
```
...
The difference is the order of the time variables, you need to choose the one that fit for your date type.

e.g. in data_1 column the date is built in format of day-month-year so I use the dmy function

```{r}
timeSample$data_1 <- dmy(timeSample$data_1)

knitr::kable(timeSample)
```

in data_2 column the date is built in format of year-month-day so i use the ymd function

```{r}
timeSample$data_2 <- ymd(timeSample$data_2)
knitr::kable(timeSample)
```

**Note** the only thing that matters is the order between the day/month/year, the separators can change (':' or '/' or '-' etc.) and lubridate can handle it...The same goes for 02 or 2, 2019 or 19 
...
```{r}
timeSample$datatime_1 <- dmy_hms(timeSample$datatime_1)
knitr::kable(timeSample)
```
...
if you don't specify the time zone, the default will be UTC. 
can cause problems later on if you do all kind of time calculations and don't understand why you get weird numbers (;


The 'force_tz' function will take the time as it and change the time zone (in the 'back office') to the required one.
its good for cases where you didn't specify the time zone for your data beforehand and now you want to fix it

```{r}
tz(timeSample$datatime_1)

timeSample$datatime_1 <- force_tz(timeSample$datatime_1, "Asia/Jerusalem")
...
```{r}
timeSample$DateTime_UTC <- with_tz(timeSample$datatime_1, "UTC")
...
### other nice function in lubridate
...
lubridate have a series of function that extract different variable out of the datetime
...
### It seems that trips with duration below 15 mins are default to zero...

lets assume that this is a mistake and change all the zero duration to a five minus drive
...
first create the boundary of each interval (need to be combination of the **date** and hour)
then use ifelse to match each trip to the proper time category
...
"floor_date" and "ceiling_date" that will round the time down or up respectively
...
the 'labels' argument control for the label of your data

as this function don't work with lubridate the syntax here is different

before each time variable need to add % sign, then the symbol for the time argument and the desired separator
...
####1. use the hms package that can extract only the time variable from the date_time
```{r}
...
the problem - this package don't work fluently with the ggplot and its not clear how to customize the axis
